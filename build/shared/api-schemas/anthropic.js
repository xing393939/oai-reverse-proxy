...
exports.containsImageContent = exports.flattenAnthropicMessages = exports.transformAnthropicTextToAnthropicChat = exports.transformOpenAIToAnthropicText = exports.transformOpenAIToAnthropicChat = exports.AnthropicV1MessagesSchema = exports.AnthropicV1TextSchema = void 0;
const zod_1 = require("zod");
const config_1 = require("../../config");
const errors_1 = require("../errors");
const openai_1 = require("./openai");
const CLAUDE_OUTPUT_MAX = config_1.config.maxOutputTokensAnthropic;
const AnthropicV1BaseSchema = zod_1.z
    .object({
    model: zod_1.z.string().max(100),
    stop_sequences: zod_1.z.array(zod_1.z.string().max(500)).optional(),
    stream: zod_1.z.boolean().optional().default(false),
    temperature: zod_1.z.coerce.number().optional().default(1),
    top_k: zod_1.z.coerce.number().optional(),
    top_p: zod_1.z.coerce.number().optional(),
    metadata: zod_1.z.object({ user_id: zod_1.z.string().optional() }).optional(),
})
    .strip();
// https://docs.anthropic.com/claude/reference/complete_post [deprecated]
exports.AnthropicV1TextSchema = AnthropicV1BaseSchema.merge(zod_1.z.object({
    prompt: zod_1.z.string(),
    max_tokens_to_sample: zod_1.z.coerce
        .number()
        .int()
        .transform((v) => Math.min(v, CLAUDE_OUTPUT_MAX)),
}));
const AnthropicV1MessageMultimodalContentSchema = zod_1.z.array(zod_1.z.union([
    zod_1.z.object({ type: zod_1.z.literal("text"), text: zod_1.z.string() }),
    zod_1.z.object({
        type: zod_1.z.literal("image"),
        source: zod_1.z.object({
            type: zod_1.z.literal("base64"),
            media_type: zod_1.z.string().max(100),
            data: zod_1.z.string(),
        }),
    }),
]));
// https://docs.anthropic.com/claude/reference/messages_post
exports.AnthropicV1MessagesSchema = AnthropicV1BaseSchema.merge(zod_1.z.object({
    ...
    system: zod_1.z
        .union([
        zod_1.z.string(),
        zod_1.z.array(zod_1.z.object({ type: zod_1.z.literal("text"), text: zod_1.z.string() })),
    ])
        .optional(),
}));
function openAIMessagesToClaudeTextPrompt(messages) {
    return (messages
        .map((m) => {
        let role = m.role;
        if (role === "assistant") {
        ...
        const content = (0, openai_1.flattenOpenAIMessageContent)(m.content);
...
const transformOpenAIToAnthropicChat = async (req) => {
    const { body } = req;
    ...
    return {
        ...
        top_p: rest.top_p,
        stop_sequences: typeof rest.stop === "string" ? [rest.stop] : rest.stop || undefined,
...
exports.transformOpenAIToAnthropicChat = transformOpenAIToAnthropicChat;
const transformOpenAIToAnthropicText = async (req) => {
    const { body } = req;
    const result = openai_1.OpenAIV1ChatCompletionSchema.safeParse(body);
    if (!result.success) {
        req.log.warn({ issues: result.error.issues, body }, "Invalid OpenAI-to-Anthropic Text request");
        throw result.error;
    }
    const { messages, ...rest } = result.data;
...
exports.transformOpenAIToAnthropicText = transformOpenAIToAnthropicText;
/**
 * Converts an older Anthropic Text Completion prompt to the newer Messages API
 * by splitting the flat text into messages.
 */
const transformAnthropicTextToAnthropicChat = async (req) => {
    const { body } = req;
    const result = exports.AnthropicV1TextSchema.safeParse(body);
    if (!result.success) {
        req.log.warn({ issues: result.error.issues, body }, "Invalid Anthropic Text-to-Anthropic Chat request");
        throw result.error;
    }
...
exports.transformAnthropicTextToAnthropicChat = transformAnthropicTextToAnthropicChat;
function validateAnthropicTextPrompt(prompt) {
    if (!prompt.includes("\n\nHuman:") || !prompt.includes("\n\nAssistant:")) {
        throw new errors_1.BadRequestError("Prompt must contain at least one human and one assistant message.");
    }
    ...
    if (firstAssistant < firstHuman) {
        throw new errors_1.BadRequestError("First Assistant message must come after the first Human message.");
    }
}
...
exports.flattenAnthropicMessages = flattenAnthropicMessages;
function openAIMessagesToClaudeChatPrompt(messages) {
...
function isSystemOpenAIRole(role) {
...
function containsImageContent(messages) {
    return messages.some(({ content }) => typeof content !== "string" && content.some((c) => c.type === "image"));
}
