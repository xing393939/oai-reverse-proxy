...
exports.OpenAIKeyChecker = void 0;
...
class OpenAIKeyChecker extends key_checker_base_1.KeyCheckerBase {
    cloneKey;
    constructor(keys, cloneFn, updateKey) {
        super(keys, {
            service: "openai",
    ...
    async getProvisionedModels(key) {
        ...
        data.data.forEach(({ id }) => {
            ids.add(id);
            families.add((0, models_1.getOpenAIModelFamily)(id, "turbo"));
        });
    ...
    async maybeCreateOrganizationClones(key) {
        if (key.organizationId)
            return; // already cloned
        try {
            const opts = { headers: { Authorization: `Bearer ${key.key}` } };
            const { data } = await axios.get(GET_ORGANIZATIONS_URL, opts);
            const organizations = data.data;
            const defaultOrg = organizations.find(({ is_default }) => is_default);
            this.updateKey(key.hash, { organizationId: defaultOrg?.id });
            if (organizations.length <= 1)
                return;
            this.log.info({ parent: key.hash, organizations: organizations.map((org) => org.id) }, "Key is associated with multiple organizations; cloning key for each organization.");
            ...
            if (status === 401) {
                ...
                switch (data.error.type) {
                    ...
                    case "access_terminated":
                        ...
                        const modelFamilies = isRevoked
                            ? ["turbo"]
                            : key.modelFamilies;
                        this.log.warn({ key: key.hash, rateLimitType: data.error.type, error: data }, "Key returned a non-transient 429 error. Disabling key.");
                        this.updateKey(key.hash, {
                            isDisabled: true,
                            isRevoked,
                            isOverQuota,
                    ...
                    case "requests":
                        ...
                        const isTrial = Number(error.response.headers["x-ratelimit-limit-requests"]) <=
                            250;
                        if (isTrial) {
                            this.log.warn({ key: key.hash, error: data }, "Trial key is rate limited on text completion endpoint. This indicates the key is being used by several proxies at once and is not likely to be usable. Disabling key.");
                            this.updateKey(key.hash, {
                                isTrial,
                                isDisabled: true,
                                isOverQuota: true,
                            ...
                            this.log.warn({ key: key.hash, error: data }, "Non-trial key is rate limited on text completion endpoint. This is unusual and may indicate a bug. Assuming key is operational.");
                    ...
                    case "tokens":
                        ...
                        this.log.info({ key: key.hash }, "Key is currently `tokens` rate limited; assuming it is operational.");
                        this.updateKey(key.hash, { lastChecked: Date.now() });
                        break;
                    default:
                        this.log.error({ key: key.hash, rateLimitType: data.error.type, error: data }, "Encountered unexpected rate limit error class while checking key. This may indicate a change in the API; please report this.");
    ...
    async testLiveness(key) {
        ...
        const { headers, data } = await axios.post(POST_CHAT_COMPLETIONS_URL, payload, {
    ...
    static errorIsOpenAIError(error) {
...
exports.OpenAIKeyChecker = OpenAIKeyChecker;
