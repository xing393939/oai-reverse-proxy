...
const GET_INVOCATION_LOGGING_CONFIG_URL = (region) => `https://bedrock.${region}.amazonaws.com/logging/modelinvocations`;
const GET_LIST_INFERENCE_PROFILES_URL = (region) => `https://bedrock.${region}.amazonaws.com/inference-profiles?maxResults=1000`;
const POST_INVOKE_MODEL_URL = (region, model) => `https://${AMZ_HOST.replace("%REGION%", region)}/model/${model}/invoke`;
const TEST_MESSAGES = [
    { role: "user", content: "Hi!" },
    { role: "assistant", content: "Hello!" },
];
class AwsKeyChecker extends key_checker_base_1.KeyCheckerBase {
    ...
    async testKeyOrFail(key) {
        ...
        if (isInitialCheck) {
            try {
                ...
                this.log.warn({ key: key.hash, error: e.message, data }, "Cannot list inference profiles.\n\
...
See https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference-prereq.html.");
        ...
        // TODO: use allsettled
        const results = await Promise.all(KNOWN_MODEL_IDS.filter(([model]) => 
        // Skip checks for models that are disabled anyway
        config_1.config.allowedModelFamilies.includes((0, models_1.getAwsBedrockModelFamily)(model))).map(async ([model, ...aliases]) => ({
        ...
        this.updateKey(key.hash, {
            modelIds,
            modelFamilies: Array.from(new Set(modelIds.map(models_1.getAwsBedrockModelFamily))),
        });
        this.log.info({
            key: key.hash,
    ...
    handleAxiosError(key, error) {
        if (error.response && AwsKeyChecker.errorIsAwsError(error)) {
            const errorHeader = error.response.headers["x-amzn-errortype"];
            ...
            switch (errorType) {
                case "AccessDeniedException":
                    ...
                    const path = new url_1.URL(error.config?.url).pathname;
                    const data = error.response.data;
                    this.log.warn({ key: key.hash, type: errorType, path, data }, "Key can't perform a required action; disabling.");
                    return this.updateKey(key.hash, { isDisabled: true });
                case "UnrecognizedClientException":
                    // This is a 403 error that indicates the key is revoked.
                    this.log.warn({ key: key.hash, errorType, error: error.response.data }, "Key is revoked; disabling.");
                ...
                case "ThrottlingException":
                    ...
                    this.log.warn({ key: key.hash, errorType, error: error.response.data }, "Key is rate limited. Rechecking in 30 seconds.");
                ...
                default:
                    ...
                    this.log.error({ key: key.hash, errorType, error: error.response.data }, "Encountered unexpected error while checking key. This may indicate a change in the API; please report this.");
                    return this.updateKey(key.hash, { lastChecked: Date.now() });
            }
        }
        const { response } = error;
        const { headers, status, data } = response ?? {};
        this.log.error({ key: key.hash, status, headers, data, error: error.message }, "Network error while checking key; trying this key again in a minute.");
    ...
    async invokeModel(model, key) {
        if (model.includes("claude")) {
            ...
            const profile = key.inferenceProfileIds.find((id) => `${continent}.${model}` === id);
            if (profile) {
                this.log.debug({ key: key.hash, model, profile }, "Testing model via inference profile.");
                let result;
                try {
                    result = await this.testClaudeModel(key, profile);
                }
                catch (e) {
                    this.log.error({ key: key.hash, model, profile, error: e.message }, "InvokeModel via inference profile returned an error; trying model ID directly.");
                    result = false;
                }
    ...
    async testClaudeModel(key, model) {
        ...
        const errorType = headers["x-amzn-errortype"].split(":")[0];
        ...
        if (status === 503 && errorType.match(/ServiceUnavailableException/i)) {
            this.log.warn({ key: key.hash, model, errorType, data, status, headers }, "Model is accessible, but may be temporarily unavailable.");
            return true;
        }
        ...
        if (status === 429) {
            if (errorType.match(/ThrottlingException/i)) {
                this.log.debug({ key: key.hash, model, errorType, data, status, headers }, "Model is available but key is rate limited.");
                return true;
            }
            else {
                throw new axios_1.AxiosError(`InvokeModel returned 429 of type ${errorType}`, `AWS_INVOKE_MODEL_RATE_LIMITED`, response.config, response.request, response);
            }
        }
        ...
        if (status === 403 &&
            ...
            this.log.debug({ key: key.hash, model, errorType, data, status, headers }, "Model is not available (principal does not have access).");
            return false;
        }
        ...
        if (status === 404) {
            this.log.debug({ region: creds.region, model, key: key.hash }, "Model is not available (not supported in this AWS region).");
            return false;
        }
        ...
        if (!correctErrorType || !correctErrorMessage) {
            this.log.debug({ key: key.hash, model, errorType, data, status }, "Model is not available (request rejected).");
            return false;
        }
        this.log.debug({ key: key.hash, model, errorType, data, status }, "Model is available.");
        return true;
    }
    async testMistralModel(key, model) {
        const creds = AwsKeyChecker.getCredentialsFromKey(key);
        const payload = {
            max_tokens: -1,
        ...
        if (status === 403 || status === 404) {
            this.log.debug({ key: key.hash, model, errorType, data, status }, "Model is not available (no access or unsupported region).");
            return false;
        }
        ...
        if (isBadRequest && !isValidationError) {
            this.log.debug({ key: key.hash, model, errorType, data, status, headers }, "Model is not available (request rejected).");
            return false;
        }
        this.log.debug({ key: key.hash, model, errorType, data, status }, "Model is available.");
        return true;
    }
    async checkInferenceProfiles(key) {
        const creds = AwsKeyChecker.getCredentialsFromKey(key);
        const req = {
            method: "GET",
        ...
        const profileIds = inferenceProfileSummaries.map((p) => p.inferenceProfileId);
        this.log.debug({ key: key.hash, profileIds, region: creds.region }, "Inference profiles found.");
        this.updateKey(key.hash, { inferenceProfileIds: profileIds });
    }
    async checkLoggingConfiguration(key) {
        if (config_1.config.allowAwsLogging) {
        ...
        await AwsKeyChecker.signRequestForAws(req, key);
        const { data, status, headers } = await axios.request(req);
        let result = "unknown";
        if (status === 200) {
            const { loggingConfig } = data;
            const loggingEnabled = !!loggingConfig?.textDataDeliveryEnabled;
            this.log.debug({ key: key.hash, loggingConfig, loggingEnabled }, "AWS model invocation logging test complete.");
            result = loggingEnabled ? "enabled" : "disabled";
        }
        else {
            const errorType = headers["x-amzn-errortype"].split(":")[0];
            this.log.debug({ key: key.hash, errorType, data, status }, "Can't determine AWS model invocation logging status.");
        }
        this.updateKey(key.hash, { awsLoggingStatus: result });
        return !!result;
    }
    static errorIsAwsError(error) {
    ...
    static async signRequestForAws(axiosRequest, key, awsService = "bedrock") {
        const creds = AwsKeyChecker.getCredentialsFromKey(key);
        const { accessKeyId, secretAccessKey, region } = creds;
        const { method, url: axUrl, headers: axHeaders, data } = axiosRequest;
        const url = new url_1.URL(axUrl);
        let plainHeaders = {};
        if (axHeaders instanceof axios_1.AxiosHeaders) {
...
exports.AwsKeyChecker = AwsKeyChecker;
