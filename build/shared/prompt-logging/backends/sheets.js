...
exports.init = exports.loadLogSheet = exports.appendBatch = void 0;
const googleapis_1 = require("googleapis");
const config_1 = require("../../../config");
const logger_1 = require("../../../logger");
const MAX_ROWS_PER_SHEET = 2000;
const log = logger_1.logger.child({ module: "sheets" });
...
const loadIndexSheet = async (assertLockId = true) => {
    ...
    const spreadsheetId = config_1.config.googleSheetsSpreadsheetId;
    ...
    if (assertLockId) {
        ...
        if (lockIdCell !== lockId) {
            log.error({ receivedLock: lockIdCell, expectedLock: lockId }, "Another instance of the proxy is writing to the spreadsheet; stopping.");
            stop();
...
const appendBatch = async (batch) => {
    ...
    if (data.data.updates && data.data.updates.updatedRows) {
        ...
        activeLogSheet.rows = activeLogSheet.rows.concat(newRows.map((row) => ({
            model: row[0],
            endpoint: row[1],
            promptRaw: row[2],
        ...
        // batch.
        log.warn({ sheetName, rowCount: newRows.length }, "No updates received from append. Creating new sheet and retrying.");
        await createLogSheet();
        throw new Error("No updates received from append.");
    }
    await finalizeBatch();
};
exports.appendBatch = appendBatch;
const finalizeBatch = async () => {
    const sheetName = activeLogSheet.sheetName;
    const rowCount = activeLogSheet.rows.length;
    const indexRow = indexSheet.rows.find(({ logSheetName }) => logSheetName === sheetName);
...
/** Not currently used. */
const loadLogSheet = async ({ sheetName, fromRow = 2, // omit header row
 }) => {
    const client = sheetsClient;
    const spreadsheetId = config_1.config.googleSheetsSpreadsheetId;
...
exports.loadLogSheet = loadLogSheet;
const init = async (onStop) => {
    if (sheetsClient) {
        return;
    }
    if (!config_1.config.googleSheetsKey || !config_1.config.googleSheetsSpreadsheetId) {
        throw new Error("Missing required Google Sheets config. Refer to documentation for setup instructions.");
    }
    log.info("Initializing Google Sheets backend.");
    const encodedCreds = config_1.config.googleSheetsKey;
    // encodedCreds is a base64-encoded JSON key from the GCP console.
    const creds = JSON.parse(Buffer.from(encodedCreds, "base64").toString("utf8").trim());
    ...
    stopCallback = onStop;
    const sheetId = config_1.config.googleSheetsSpreadsheetId;
    ...
    if (!res.data) {
        ...
        log.error({
            ...
            sheetId: config_1.config.googleSheetsSpreadsheetId,
...
function assertData(res) {
    if (!res.data) {
        const { status, statusText, headers } = res;
        log.error({ res: { status, statusText, headers } }, "Unexpected response from Google Sheets API.");
    }
    return res.data;
}
