...
exports.estimateGoogleAITokenCount = exports.getOpenAIImageCost = exports.DALLE_TOKENS_PER_DOLLAR = exports.getTokenCount = exports.init = void 0;
...
const log = logger_1.logger.child({ module: "tokenizer", service: "openai" });
const GPT4_VISION_SYSTEM_PROMPT_SIZE = 170;
let encoder;
function init() {
    encoder = new lite_1.Tiktoken(cl100k_base_json_1.default.bpe_ranks, cl100k_base_json_1.default.special_tokens, cl100k_base_json_1.default.pat_str);
    return true;
}
exports.init = init;
// Tested against:
// https://github.com/openai/openai-cookbook/blob/main/examples/How_to_count_tokens_with_tiktoken.ipynb
async function getTokenCount(prompt, model) {
...
async function getGpt4VisionTokenCost(url, detail = "auto") {
    // For now we do not allow remote images as the proxy would have to download
    // them, which is a potential DoS vector.
    if (!url.startsWith("data:image/")) {
        throw new Error("Remote images are not supported. Add the image to your prompt as a base64 data URL.");
    }
    ...
    if (selectedDetail === "low") {
        log.info({ width, height, tokens: 85 }, "Using fixed GPT-4-Vision token cost for low detail image");
        return 85;
    }
    let newWidth = width;
    ...
    log.info({ width, height, newWidth, newHeight, tiles, tokens }, "Calculated GPT-4-Vision token cost for high detail image");
    return tokens;
}
...
// 512×512	$0.018 / image
// 256×256	$0.016 / image
exports.DALLE_TOKENS_PER_DOLLAR = 100000;
/**
 ...
 */
function getOpenAIImageCost(params) {
    ...
    const tokens = (n ?? 1) * (usd * exports.DALLE_TOKENS_PER_DOLLAR);
    return {
        tokenizer: `openai-image cost`,
        token_count: Math.ceil(tokens),
    };
}
exports.getOpenAIImageCost = getOpenAIImageCost;
function estimateGoogleAITokenCount(prompt) {
    if (typeof prompt === "string") {
        return getTextTokenCount(prompt);
    }
...
exports.estimateGoogleAITokenCount = estimateGoogleAITokenCount;
