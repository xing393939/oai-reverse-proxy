...
exports.addAzureKey = void 0;
const key_management_1 = require("../../../../shared/key-management");
const addAzureKey = async (manager) => {
    const req = manager.request;
    const validAPIs = ["openai", "openai-image"];
    const apisValid = [req.outboundApi, req.inboundApi].every((api) => validAPIs.includes(api));
    ...
    const key = key_management_1.keyPool.get(model, "azure");
    manager.setKey(key);
    ...
    req.log.info({ key: key.hash, model }, "Assigned Azure OpenAI key to request");
    const cred = req.key;
    const { resourceName, deploymentId, apiKey } = getCredentialsFromKey(cred);
    const operation = req.outboundApi === "openai" ? "/chat/completions" : "/images/generations";
    const apiVersion = req.outboundApi === "openai" ? "2023-09-01-preview" : "2024-02-15-preview";
...
exports.addAzureKey = addAzureKey;
function getCredentialsFromKey(key) {
...
//# sourceMappingURL=add-azure-key.js.map