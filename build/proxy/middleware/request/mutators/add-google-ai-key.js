...
exports.addGoogleAIKey = void 0;
const key_management_1 = require("../../../../shared/key-management");
const addGoogleAIKey = (manager) => {
    const req = manager.request;
    const inboundValid = req.inboundApi === "openai" || req.inboundApi === "google-ai";
    ...
    const key = key_management_1.keyPool.get(model, "google-ai");
    manager.setKey(key);
    req.log.info({ key: key.hash, model, stream: req.isStreaming }, "Assigned Google AI API key to request");
    ...
    manager.setSignedRequest({
        ...
        path: `/v1beta/models/${model}:${req.isStreaming ? "streamGenerateContent?alt=sse&" : "generateContent?"}key=${key.key}`,
        headers: {
...
exports.addGoogleAIKey = addGoogleAIKey;
//# sourceMappingURL=add-google-ai-key.js.map