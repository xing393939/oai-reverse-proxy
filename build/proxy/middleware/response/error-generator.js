...
exports.buildSpoofedSSE = exports.buildSpoofedCompletion = exports.sendErrorToClient = void 0;
 ...
 */
function redactHostname(options) {
    if (!options.message.includes("getaddrinfo"))
        return options;
 ...
 */
function sendErrorToClient(params) {
    ...
    const format = options.format === "unknown" ? tryInferFormat(req.body) : options.format;
    if (format === "unknown") {
        ...
        return res.status(code).json({
            error: {
                message,
                type: http_1.default.STATUS_CODES[code].replace(/\s+/g, "_").toLowerCase(),
            },
            ...(hasDetails ? { details } : {}),
        });
    }
 ...
 * error handling.
 */
function buildSpoofedCompletion({ format, title, message, obj, reqId, model = "unknown", }) {
...
exports.buildSpoofedCompletion = buildSpoofedCompletion;
/**
 ...
 */
function buildSpoofedSSE({ format, title, message, obj, reqId, model = "unknown", }) {
    const id = String(reqId);
    const content = getMessageContent({ title, message, obj });
    let event;
    switch (format) {
    ...
    if (format === "anthropic-text") {
        return (["event: completion", `data: ${JSON.stringify(event)}`].join("\n") +
            "\n\n");
    }
    // ugh.
...
exports.buildSpoofedSSE = buildSpoofedSSE;
